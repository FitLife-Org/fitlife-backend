...
class AiNutritionPlanOrchestratorServiceImplTest {
    ...
    @Mock private CurrentMemberService currentMemberService;
    @Mock private AiUsageService aiUsageService;
    @Mock private BodyMetricRepository bodyMetricRepository;
    @Mock private AiSnapshotService aiSnapshotService;
    @Mock private AiPromptBuilderService aiPromptBuilderService;
    @Mock private AiProviderService aiProviderService;
    @Mock private AiPlanParserService aiPlanParserService;
    @Mock private AiResponseValidatorService aiResponseValidatorService;
    @Mock private AiSuggestionPersistenceService aiSuggestionPersistenceService;
    @Mock private AiSuggestionMapper aiSuggestionMapper;

    private AiNutritionPlanOrchestratorServiceImpl orchestrator;

    @BeforeEach
    void setUp() {
        orchestrator = new AiNutritionPlanOrchestratorServiceImpl(
    ...
    void createNutritionPlan_shouldCompleteHappyPath() {
        ...
        AiNutritionPlanRequest request = createValidRequest();
        AiInputSnapshot snapshot = AiInputSnapshot.builder().build();

        AiPromptResult promptResult = AiPromptResult.builder()
                .version(AiPromptVersion.NUTRITION_PLAN_V1)
                .prompt("nutrition-prompt")
                .build();

        AiSuggestion pending = AiSuggestion.builder()
                .id(10L)
                .build();

        AiProviderResult providerResult = AiProviderResult.builder()
        ...
        generated.setWarnings(List.of("Chỉ mang tính tham khảo"));

        AiSuggestion success = AiSuggestion.builder()
                .id(10L)
                .build();
        ...
        when(currentMemberService.getCurrentMember()).thenReturn(member);
        when(bodyMetricRepository
                .findTopByMemberIdAndIsDeletedFalseOrderByRecordedAtDesc(1L))
                .thenReturn(Optional.empty());
        when(aiSnapshotService
                .buildNutritionPlanSnapshot(member, null, request))
                .thenReturn(snapshot);
        when(aiPromptBuilderService
                .buildNutritionPlanPrompt(snapshot))
                .thenReturn(promptResult);
        when(aiSuggestionPersistenceService
                .createPending(any(AiSuggestion.class)))
                .thenReturn(pending);
        when(aiProviderService.generate("nutrition-prompt"))
                .thenReturn(providerResult);
        when(aiPlanParserService.parseNutritionPlan("{}"))
                .thenReturn(generated);
        when(aiSuggestionPersistenceService.markNutritionPlanSuccess(
                10L,
                providerResult,
                generated,
        ...
        ordered.verify(aiUsageService).validateDailyLimit(1L);
        ...
        ordered.verify(aiResponseValidatorService)
                .validateNutritionPlan(generated, snapshot);
        ordered.verify(aiSuggestionPersistenceService)
